...
export default function SearchScreen() {
    ...
    const handleSearch = (query) => {
        if (query && query.trim().length > 0) {
            navigation.navigate('SearchResult', { query: query.trim() });
        }
    };
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View className="px-6 mb-6">
                    <View className="flex-row items-center bg-white rounded-full px-4 h-12 shadow-sm border border-slate-100">
                        ...
                        <TouchableOpacity onPress={() => navigation.navigate('SearchPersonalization')}>
                ...
                <View className="px-6">
                    ...
                    <View className="flex-row flex-wrap justify-between gap-y-4">
                        {explorePlaces.map((place) => (
                            <TouchableOpacity
                                key={place.id}
                                className="w-[48%] bg-white p-2 rounded-2xl shadow-sm border border-slate-100"
                                onPress={() => handleSearch(place.title)}
                            >
